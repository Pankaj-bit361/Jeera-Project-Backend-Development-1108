...
import { FiX, FiSend, FiClock, FiUser, FiZap, FiMessageSquare, FiCalendar } from 'react-icons/fi';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { getInitials, cn, calculateSprint } from '../../lib/utils';
...
const TaskDetailModal = ({ isOpen, onClose, task, members, onSave, onUpdate }) => {
  ...
  // Dynamic Sprint Calculation for UI
  const predictedSprint = currentOrg ? calculateSprint(currentOrg.createdAt) : 1;
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[90vh] animate-in zoom-in-95 duration-200">
          ...
          <form id="taskForm" onSubmit={handleSubmit} className="space-y-6">
            <div className="flex justify-between items-start">
                <Input 
                    ...
                    required 
                />
                <div className="hidden md:flex items-center gap-2 bg-indigo-50 px-3 py-1 rounded-full text-xs font-bold text-indigo-600 border border-indigo-100">
                    Sprint {task?.sprintIndex || predictedSprint}
                </div>
            </div>
        ...
        {task ? (
          ...
          // In Create Mode, show a placeholder
          <div className="hidden md:flex w-80 bg-gray-50 border-l border-gray-200 flex-col items-center justify-center p-8 text-center relative">
